...
const Demografi = () => {
  ...
  const {
    ...
    "https://api.desarandusangakulon.com/demografi",
    fetcher
  );
  const {
    data: educationData,
    ...
    "https://api.desarandusangakulon.com/education-options",
    fetcher
  );
  const {
    data: religionData,
  ...
  } = useSWR("https://api.desarandusangakulon.com/agama", fetcher);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await axiosJWT.put(
          `https://api.desarandusangakulon.com/demografi/${currentDemographic.nik}`,
          dataToSend,
          {
        ...
        await axiosJWT.post(
          "https://api.desarandusangakulon.com/cdemografi",
          dataToSend,
          {
            headers: { "Content-Type": "multipart/form-data" },
          }
      ...
      await mutate("https://api.desarandusangakulon.com/demografi");
      resetForm();
      setDialogVisible(false);
    } catch (error) {
      handleError(error);
  ...
  const editDemographic = (demographic) => {
    ...
    const fileUrl = demographic.file_url
      ? `https://api.desarandusangakulon.com/${demographic.file_url}`
      : null;
    ...
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await axiosJWT.delete(
          `https://api.desarandusangakulon.com/demografi/${nik}`
        );
        toast.current.show({
          severity: "success",
          summary: "Success",
  ...
  useEffect(() => {
    if (selectedDemographic?.file_url && !selectedFile) {
      ...
      setPreview(
        `https://api.desarandusangakulon.com/${selectedDemographic.file_url}`
      );
